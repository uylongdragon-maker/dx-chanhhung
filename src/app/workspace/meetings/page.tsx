...
export default async function MeetingsPage() {
  ...
  const [allRequests, calendarEvents] = await Promise.all([
    ...
    prisma.meetingEvent.findMany({
      where: { startTime: { gte: yearStart, lte: yearEnd } },
      include: {
    ...
    <div className="flex flex-col gap-8 pb-24 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
        <div>
          ...
          <h1 className="text-3xl font-black text-slate-800 dark:text-slate-100 tracking-tight uppercase">Quản Lý Lịch Họp</h1>
          <p className="text-slate-500 text-xs font-bold mt-1">Đặt lịch, xem lịch & đăng ký tham dự các cuộc họp</p>
        </div>
        {pending > 0 && isAdmin && (
          <div className="flex items-center gap-2 px-4 py-2 bg-amber-500/10 border border-amber-500/20 rounded-2xl">
            <div className="w-2 h-2 bg-amber-500 rounded-full animate-pulse" />
            <span className="text-xs font-black text-amber-600">{pending} yêu cầu phòng chờ duyệt</span>
          </div>
        )}
      </div>
      ...
      <section>
        <h2 className="font-black text-slate-800 dark:text-slate-100 text-base uppercase tracking-tight flex items-center gap-2 mb-5">
          <MapPin className="text-blue-500" size={18} /> Đặt phòng họp vật lý
        </h2>
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          {/* Request list */}
          <div className="lg:col-span-8 flex flex-col gap-5">
            <h3 className="font-black text-slate-700 dark:text-slate-200 text-sm uppercase tracking-tight flex items-center gap-2">
              <Clock className="text-blue-500" size={16} /> Lịch sử đặt phòng
            </h3>
