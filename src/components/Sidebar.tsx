...
import { LayoutDashboard, Kanban, Table, Users, BookOpen, Calendar, MessageSquare, Settings, ShieldCheck, LogOut, FileText } from "lucide-react";
import NotificationCenter from "./NotificationCenter";

export default function Sidebar({ user, appLogo }: { user: any; appLogo?: string }) {
  const pathname = usePathname();
  const isAdmin = user?.role === 'ADMIN';

  const navLinks = [
    { href: "/workspace",          icon: LayoutDashboard, title: "Tổng quan" },
    { href: "/workspace/table",    icon: Table,           title: "Bảng Theo Dõi" },
    ...
    { href: "/workspace/meeting-minutes", icon: FileText,   title: "Biên Bản Họp" },
    { href: "/workspace/chat",     icon: MessageSquare,    title: "Phòng Chat" },
    { href: "/workspace/settings", icon: Settings,         title: "Cài đặt" },
  ];

  return (
    <nav className="hidden md:flex h-[calc(100vh-2rem)] w-64 shrink-0 m-4 border border-white/20 dark:border-slate-800/20 bg-white/30 dark:bg-slate-950/20 backdrop-blur-2xl flex-col gap-6 py-6 px-4 shadow-[0_8px_32px_0_rgba(0,0,0,0.03)] rounded-[2.5rem] z-50">
      {/* Logo */}
      <div className="flex items-center gap-3 px-2 mb-2">
        {appLogo ? (
          <div className="w-12 h-12 rounded-2xl overflow-hidden border border-[#7360f2]/20 shadow-lg shrink-0">
            <img src={appLogo} alt="App Logo" className="w-full h-full object-cover" />
          </div>
        ) : (
          <div className="w-12 h-12 rounded-2xl bg-[#7360f2] flex items-center justify-center text-white shadow-xl shadow-[#7360f2]/10 shrink-0">
            <span className="font-black text-sm tracking-tighter">CHX</span>
          </div>
        )}
        <div className="flex flex-col">
          <span className="font-black text-slate-800 dark:text-slate-100 uppercase tracking-tight text-sm leading-none mb-1">CHX</span>
          <span className="text-[10px] font-bold text-[#7360f2] uppercase tracking-wider leading-none">WORKSPACE</span>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-2 w-full flex-grow overflow-y-auto scrollbar-hide py-2">
        {navLinks.map((link) => {
          ...
          return (
            <Link key={link.href} href={link.href} prefetch={false} className={`group flex items-center gap-3 px-4 py-3 rounded-2xl transition-all duration-300 ${isActive ? "bg-[#7360f2] text-white shadow-lg shadow-[#7360f2]/30" : "text-slate-500 hover:text-[#7360f2] hover:bg-[#7360f2]/5 dark:hover:bg-slate-900"}`}>
