  ...
  return (
    <>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        {/* Tabs */}
        <div className="flex bg-slate-100 dark:bg-slate-800 rounded-2xl p-1 gap-1 shrink-0">
          <button onClick={() => setActiveTab('board')} className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'board' ? 'bg-white dark:bg-slate-700 text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
            <CheckSquare size={14} /> Bảng việc
          </button>
          <button onClick={() => setActiveTab('pool')} className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all relative ${activeTab === 'pool' ? 'bg-white dark:bg-slate-700 text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
        ...
        {activeTab === 'board' && (
          <div className="flex gap-2 flex-1 flex-wrap">
            <div className="relative flex-1 min-w-[160px]">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Tìm việc..." className="w-full pl-8 pr-3 py-2 text-sm bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:ring-2 focus:ring-blue-500/20" />
            </div>
            <select value={filterAssignee} onChange={e => setFilterAssignee(e.target.value)} className="px-3 py-2 text-xs font-bold bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl outline-none text-slate-600 dark:text-slate-300">
              <option value="">Tất cả thành viên</option>
              {users.map(u => <option key={u.id} value={u.id}>{u.name}</option>)}
            </select>
            <select value={filterPriority} onChange={e => setFilterPriority(e.target.value)} className="px-3 py-2 text-xs font-bold bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl outline-none text-slate-600 dark:text-slate-300">
        ...
        {/* Overdue alert */}
        {overdueCount > 0 && (
          <div className="flex items-center gap-2 px-3 py-2 bg-rose-500/10 border border-rose-500/20 rounded-2xl shrink-0">
                  ...
                  <Droppable droppableId={col.status}>
                    {(provided, snapshot) => (
                      <div {...provided.droppableProps} ref={provided.innerRef}
                        className={`flex flex-col gap-3 min-h-[400px] p-3 rounded-[2rem] transition-colors duration-150 ${snapshot.isDraggingOver ? 'bg-blue-50/60 dark:bg-blue-900/10' : 'bg-slate-100/30 dark:bg-slate-950/20'} border-2 border-dashed ${snapshot.isDraggingOver ? 'border-blue-300' : 'border-slate-200/60 dark:border-slate-700/40'}`}>
                                ...
                                <div ref={provided.innerRef} {...provided.draggableProps}
                                  ...
                                  <div className="flex items-start justify-between gap-2 mb-2">
                                    <div className="flex items-center gap-2 flex-1 min-w-0">
                                      ...
                                      <h4 className={`font-bold text-slate-800 dark:text-slate-100 text-[13px] leading-snug truncate ${task.status === 'DONE' ? 'line-through opacity-40' : ''}`}>{task.title}</h4>
                                    </div>
                                  ...
                                  <div className="ml-5 flex items-center gap-2">
                                    {task.assignee ? (
                                      <div className="w-7 h-7 rounded-xl bg-gradient-to-tr from-blue-600 to-indigo-400 border-2 border-white/60 shadow-sm flex items-center justify-center text-[9px] text-white font-black overflow-hidden">
                                      ...
                                      <div className="w-7 h-7 rounded-xl bg-slate-100 dark:bg-slate-800 border-2 border-dashed border-slate-300 dark:border-slate-600 flex items-center justify-center">
