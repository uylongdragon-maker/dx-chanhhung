...
import { MoreHorizontal, Plus, CheckCircle2, Circle, Clock, Loader2, GripVertical } from "lucide-react";
...
export default function KanbanBoard({ 
  ...
  // Đồng bộ state khi props thay đổi (ví dụ sau khi revalidate)
  useEffect(() => {
    setTasks(inputTasks);
  }, [inputTasks]);
  ...
  const onDragEnd = async (result: any) => {
    ...
    // Logic cập nhật state local để người dùng thấy mượt
    ...
    // Re-calculate order cho tất cả các task trong cột đích
    const finalTasks = [
                      ...
                      <Draggable key={task.id} draggableId={task.id} index={index}>
                        {(provided, snapshot) => (
                          <div 
                            ...
                            className={`bg-white/60 dark:bg-slate-900/60 backdrop-blur-2xl border border-white/80 dark:border-slate-800/60 p-5 rounded-[2rem] shadow-sm transition-all group/card ${
